		...
		this.each(function(i, cont) {
			...
			var newList = {
				...
				grabItem: function(e) {
					...
					var list = lists[$(this).attr("data-listidx")];
					var item = this;
					if (list.lock == true)	return;//如果锁住，则不允许拖动
				...
				dropItem: function() {//修改了这里，拖动完成后，有个缓冲的效果
					if (list.draggedItem == null)
						return;
					$(list.draggedItem).click(function(event){//阻止时间冒泡
						return false;
					});
					var father = $(list.container).offset();
					var offset = $(list.placeHolderItem).offset();
					var curIndex = $(list.container).attr('data-listidx');
					var ml = parseInt($(list.container).css('marginLeft'));
					var mt = parseInt($(list.container).css('marginTop'));
					ml = isNaN(ml) ? 0 : ml;
					mt = isNaN(mt) ? 0 : mt;
					var outerWidth = $(list.container).outerWidth(true);
					var offsetFatherWidth = outerWidth*curIndex;
					$(list.draggedItem).attr('position','absolute');
					var newLeft = offsetFatherWidth + offset.left-  father.left + ml;
					var newTop = offset.top - father.top + mt;
					// console.log(ml + '/'+mt +'/'+offsetFatherWidth)
