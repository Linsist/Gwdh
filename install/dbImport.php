...
class DbManage {
    ...
    public $record;
    public $msg; // 提示信息
    public $result = false; //默认执行结果为false
    // 换行符
    private $ds = "\n";
    // 存储SQL的变量
    public $sqlContent = "";
    ...
    function backup($tablename = '', $dir='', $size=2000) {
        ...
        $size = (is_numeric($size) && $size > 0) ? $size : 2000;
        $sql = '';
        // 只备份某个表,
        // tip:这个功能我测试
        if (!empty($tablename)) {
            ...
            while ($table = mysql_fetch_array($tables)) {
                ...
                $sql .= $this->batch_insert_record($tablename, $data_list);
                // 如果大于分卷大小，则写入文件
                if (strlen($sql) >= $size * 1000) {
                    $file = $filename . "_v" . $p . ".sql";
                    // 写入文件
    ...
    /**
     *  批量插入记录
     * @param string $table 单个表名
     * @param array $data 该表全部数据
     * @return string 插入的sql语句
     */
    private function batch_insert_record($table, $data) {
        $table_name = $table;
        if (!empty($data)) {
            $fields = array();
            $values = array();
            foreach ($data as $val) {
                ...
                $one_data_value = array_values($val);
                foreach ($one_data_value as &$value) {
                    $value = mysql_real_escape_string($value);
                }
                $values[] = "('" . implode("','", $one_data_value) . "')";
            }
