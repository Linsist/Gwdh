 ...
 * @param string $regip
 */
function uc_user_register($studentNumber,$username,$campus,$academy,$grade,$major,$block,$room,$email='',$regip='') {
	return call_user_func(UC_API_FUNC, 'user', 'register', array('studentNumber'=>$studentNumber, 'campus'=>$campus,'academy'=>$academy,'grade'=>$grade,'major'=>$major,'block'=>$block,'room'=>$room,  'username'=>$username,  'email'=>$email, 'regip' => $regip));
}
 ...
 */
function uc_user_info_edit($uid, $username, $studentnumber, $academy, $major, $usertype=1) {
	return call_user_func(UC_API_FUNC, 'user', 'info_edit', array('uid'=>$uid, 'username'=>$username, 'studentnumber'=>$studentnumber, 'academy'=>$academy, 'major'=>$major, 'usertype'=>$usertype));
}
 ...
 * @return -1 没做任何更改, 1更改成功 ,0更改失败
 */
function uc_user_setlogininfo($uid,$lastloginip,$lastlogintime){
	return call_user_func(UC_API_FUNC,'user','setlogininfo',array('uid'=>$uid,'lastloginip'=>$lastloginip,'lastlogintime'=>$lastlogintime));
}
...
/**
 * 获取用户信息
 * @param string $username 选择字段
 * @param int $type 可选0,1,2; 0为studentNumber(默认) ,1为uid, 2为email
 */
function uc_get_user($username, $type=0){
	$return = call_user_func(UC_API_FUNC, 'user', 'get_user', array('username'=>$username, 'type'=>$type));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
