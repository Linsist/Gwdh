...
class usermodel {
	...
	/**
	 * 通过学号获得信息
	 * @param unknown_type $studentNumber
	 */
	function get_user_by_student_number($studentNumber) {
		$arr = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE studentNumber='$studentNumber'");
		return $arr;
	}
	 ...
	 * @param string $regip
	 */
	function gw_add_user($studentNumber,$username,$campus,$academy,$grade,$major,$block,$room,$email='',$regip=''){
		$regip = empty($regip) ? $this->base->onlineip : $regip;
		
		$sql = "INSERT INTO ".UC_DBTABLEPRE."members SET studentNumber='$studentNumber', username='$username',";
		$sql .= "campus='$campus', academy='$academy', grade='$grade', major='$major', block='$block',";
		$sql .= "room='$room', email='$email', regip='$regip'"; 
	 ...
	 * @param string $academy
	 * @param string $major
	 * @param int $usertype 1是普通用户 ,2为quanta用户
	 * return 1更新成功; -7没做任何更改; -8 用户受保护，无权限更改; 
	 */
	function edit_user_info($uid,$username, $studentnumber, $academy, $major, $usertype = 1){
		...
		$sqladd = $username? "username='".$username."'" : '';
		$sqladd .= $studentnumber ? ($sqladd ? ',' : '')."studentnumber='".$studentnumber."'" : '';
		$sqladd .= $academy ? ($sqladd ? ',' : '')."academy='".$academy."'" : '';
		//$sqladd .= $major ? ($sqladd ? ',' : '')."major='$major'" : '';
		$sqladd .= ($sqladd ? ',' : '')."major='$major'";
		$sqladd .= $usertype ? ($sqladd ? ',' : '')."usertype='$usertype'" : '';
	 ...
	 * @return -1没做任何更改
	 */
	function setlogininfo($uid,$lastloginip,$lastlogintime){
		$sqladd = $lastloginip ? ("lastloginip='".$lastloginip."'") : '';
		$sqladd .= $lastlogintime ? ($sqladd ? ',':'')."lastlogintime='".$lastlogintime."'" : '' ;
		if($sqladd){
			$query = $this->db->query("UPDATE ".UC_DBTABLEPRE."members SET $sqladd WHERE uid='$uid'");
