...
//	}
	...
	function onregister() {
		...
		$email = $this->input('email');
		$regip = $this->input('regip');
		$studentNumber = $this->input('studentNumber');
		$campus = $this->input('campus');
		$academy = $this->input('academy');
		...
		$uid = $_ENV['user']->gw_add_user($studentNumber,$username,$campus,$academy,$grade,$major,$block,$room,$email,$regip);
		return $uid;
	}
	...
	/**
	 * 自己添加的，更新用户信息的，可以通过id更新用户名
	 * Enter description here ...
	 */
	function oninfo_edit(){
		$this->init_input();
		$uid = $this->input('uid');
		$username = $this->input('username');
		$studentnumber = $this->input('studentnumber');
		...
		$status = $_ENV['user']->edit_user_info($uid,$username, $studentnumber, $academy, $major, $usertype);
...
//		return $user;
	...
	function onget_user() {
		...
		if($type==0) {
			$status = $_ENV['user']->get_user_by_student_number($username);
		} else if($type==1) {
			$status = $_ENV['user']->get_user_by_uid($username);
		}else{
	 ...
	 * 
	 * 设置最后登录的ip 和 time
	 * @return -1 则没做任何更改 ，>0时返回的是受影响到条数
	 */
	function onsetlogininfo(){
		$this->init_input();
		$uid = $this->input('uid');
		$lastloginip = $this->input('lastloginip');
		$lastlogintime = $this->input('lastlogintime');
		
		$statues = $_ENV['user']->setlogininfo($uid,$lastloginip,$lastlogintime);
		return $statues;
	}
